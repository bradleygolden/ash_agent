defmodule AshAgent.Integration.Metadata.ReqLLM.OllamaTest do
  @moduledoc false
  ...
  defmodule TestAgent do
    @moduledoc false
    use Ash.Resource,
      domain: AshAgent.Integration.Metadata.ReqLLM.OllamaTest.TestDomain,
  ...
  describe "unified metadata extraction" do
    ...
    test "metadata contains provider identifier" do
      assert {:ok, %Result{metadata: metadata}} =
               AshAgent.Runtime.call(TestAgent, %{message: "provider test"})
    ...
    test "Result contains usage statistics" do
      assert {:ok, %Result{usage: usage}} =
               AshAgent.Runtime.call(TestAgent, %{message: "usage test"})
    ...
    test "Result contains model identifier" do
      assert {:ok, %Result{model: model}} =
               AshAgent.Runtime.call(TestAgent, %{message: "model test"})
    ...
    test "Result contains finish_reason" do
      assert {:ok, %Result{finish_reason: finish_reason}} =
               AshAgent.Runtime.call(TestAgent, %{message: "finish test"})
  ...
  describe "telemetry integration" do
    test "telemetry events include provider metadata" do
      ...
      try do
        {:ok, _result} =
          AshAgent.Runtime.call(TestAgent, %{message: "telemetry test"})
  ...
  describe "metadata serializability for persistence" do
    test "Result.metadata is JSON-encodable" do
      {:ok, result} = AshAgent.Runtime.call(TestAgent, %{message: "serialize test"})
    ...
    test "Result.usage is JSON-encodable" do
      {:ok, result} = AshAgent.Runtime.call(TestAgent, %{message: "usage serialize"})
    ...
    test "full Result can be serialized for state capture" do
      {:ok, result} = AshAgent.Runtime.call(TestAgent, %{message: "state capture"})
  ...
  describe "context reconstruction for session continuity" do
    test "result.context can be used for next turn" do
      {:ok, result} = AshAgent.Runtime.call(TestAgent, %{message: "context test"})
    ...
    test "context messages are serializable for persistence" do
      {:ok, result} = AshAgent.Runtime.call(TestAgent, %{message: "context serialize"})
  ...
  describe "usage accumulation for budget tracking" do
    test "usage has required fields for accumulation" do
      {:ok, result} = AshAgent.Runtime.call(TestAgent, %{message: "budget test"})
    ...
    test "usage values are summable integers" do
      {:ok, result} = AshAgent.Runtime.call(TestAgent, %{message: "sum test"})
  ...
  describe "telemetry and result metadata consistency" do
    test "telemetry stop event contains same usage as Result" do
      ...
      try do
        {:ok, result} = AshAgent.Runtime.call(TestAgent, %{message: "consistency"})
    ...
    test "telemetry contains status on success" do
      ...
      try do
        {:ok, _result} = AshAgent.Runtime.call(TestAgent, %{message: "status test"})
  ...
  describe "runtime timing capture" do
    test "metadata includes started_at timestamp" do
      {:ok, result} = AshAgent.Runtime.call(TestAgent, %{message: "timing test"})
